...
const rolePermissions: Record<string,string[]> = {
  'Super Admin':['*'],
  'Admin':['people','attendance','schedule','leave','payroll','talent','reports'],
  'HRD':['people','attendance','schedule','leave','talent','reports'],
  'Payroll':['people.read','attendance.read','payroll','reports'],
  'Supervisor':['attendance','schedule','leave','reports'],
  'Karyawan':[]
};
const menuPermissionForRole=(role:string,key:string)=>{
 const p=rolePermissions[role]||[]; if(p.includes('*')||key==='overview') return true;
 const root=key.split('-')[0];
 if(root==='employees'||root==='employee') return p.includes('people');
 if(['attendance','late','overtime','selfie'].includes(root)) return p.includes('attendance');
 if(['schedule','shift','holiday'].includes(root)) return p.includes('schedule');
 if(['leave'].includes(root)) return p.includes('leave');
 if(['payroll','payslip'].includes(root)) return p.includes('payroll');
 if(['performance','kpi','recruitment','candidates'].includes(root)) return p.includes('talent');
 if(root==='reports') return p.includes('reports');
...
export default function DashboardAdmin(){
 ...
 useEffect(()=>{supabase.auth.getUser().then(async({data})=>{if(data.user){const {data:p}=await supabase.from('hris_users').select('role,status').eq('email',data.user.email||'').maybeSingle();if(p&&p.status==='Aktif'&&Object.prototype.hasOwnProperty.call(rolePermissions,p.role)){setUserRole(p.role);setLogged(true)}}})},[]);
 ...
 async function login(e:FormEvent){
  ...
  if(pe || !profile || profile.status!=='Aktif' || !Object.prototype.hasOwnProperty.call(rolePermissions,profile.role)){

    await signOut(); setError('Akun tidak memiliki akses Dashboard HR.'); return;
  }
  setUserRole(profile.role); setLogged(true);
 }
 ...
 return <div className="talenta-shell">
  <aside className={`talenta-sidebar ${sidebar?'':'collapsed'}`}>
   ...
   <nav className="sidebar-nav">{menuGroups.map(g=><div className="nav-group" key={g.title}>
    {sidebar&&<button className="group-title" onClick={()=>setOpen(v=>({...v,[g.title]:!v[g.title]}))}><span>{g.title}</span><span>{open[g.title]?'⌄':'›'}</span></button>}
    {(sidebar?open[g.title]:true)&&g.items.filter(([key])=>menuPermissionForRole(userRole,key)).map(([key,label,icon])=><button key={key} className={`nav-item ${menu===key?'active':''}`} onClick={()=>setMenu(key as MenuKey)} title={label}><span className="nav-icon">{icon}</span>{sidebar&&<span>{label}</span>}{key==='employees'&&sidebar&&<em>{employees.length}</em>}</button>)}
   </div>)}</nav>
   <div className="sidebar-bottom"><div className="admin-mini"><div className="avatar">HR</div>{sidebar&&<div><b>Administrator HR</b><small>Super Admin</small></div>}</div><button className="logout" onClick={async()=>{await signOut();setLogged(false)}}>↪ {sidebar&&'Keluar'}</button></div>
  </aside>
...
function Settings(){const [f,setF]=useState<any>({company_name:'Moonjustfine by Tirta',work_start:'07:00',work_end:'16:00',break_minutes:60,payday_day:'Jumat',currency:'IDR',timezone:'Asia/Jakarta'}),[msg,setMsg]=useState('');useEffect(()=>{supabase.from('hris_company_settings').select('*').eq('id',1).maybeSingle().then(({data})=>data&&setF(data))},[]);const save=async()=>{const {error}=await supabase.from('hris_company_settings').upsert({...f,id:1});setMsg(error?error.message:'Pengaturan tersimpan.');};return <><Heading title="Pengaturan" desc="Konfigurasi perusahaan yang benar-benar disimpan." action="Simpan Perubahan" onAction={save}/>{msg&&<div className="alert">{msg}</div>}<div className="panel form-panel"><div className="form-grid">{Object.entries(f).filter(([k])=>k!=='id'&&k!=='updated_at').map(([k,v])=><label key={k}>{fieldLabel(k)}<input type={k.includes('minutes')?'number':k.includes('start')||k.includes('end')?'time':'text'} value={String(v??'')} onChange={e=>setF({...f,[k]:k.includes('minutes')?Number(e.target.value):e.target.value})}/></label>)}</div></div></>}
function Roles(){const [rows,setRows]=useState<any[]>([]),[open,setOpen]=useState(false),[nama,setNama]=useState(''),[desc,setDesc]=useState('');const load=async()=>{const {data}=await supabase.from('hris_roles').select('*').order('nama');setRows(data||[])};useEffect(()=>{load()},[]);const save=async(e:FormEvent)=>{e.preventDefault();const {error}=await supabase.from('hris_roles').insert({nama,deskripsi:desc});if(error)alert(error.message);else{setOpen(false);setNama('');setDesc('');load()}};return <><Heading title="Role & Permission" desc="Role yang tersimpan di database." action="＋ Tambah Role" onAction={()=>setOpen(true)}/><div className="panel table-panel"><div className="table-wrap"><table><thead><tr><th>Role</th><th>Deskripsi</th><th>Dibuat</th></tr></thead><tbody>{rows.length?rows.map(r=><tr key={r.id}><td><b>{r.nama}</b></td><td>{r.deskripsi||'-'}</td><td>{r.created_at?.slice(0,10)||'-'}</td></tr>):<Empty cols={3}/>}</tbody></table></div></div>{open&&<SimpleModal title="Tambah Role" onClose={()=>setOpen(false)} onSave={save}><label>Nama Role<input required value={nama} onChange={e=>setNama(e.target.value)}/></label><label>Deskripsi<textarea value={desc} onChange={e=>setDesc(e.target.value)}/></label></SimpleModal>}</>}
