...
export default function DashboardAdmin(){
 ...
 useEffect(()=>{supabase.auth.getUser().then(async({data})=>{if(data.user){const {data:p}=await supabase.from('hris_users').select('role,status').eq('email',data.user.email||'').maybeSingle();if(p&&p.status==='Aktif'&&['Admin','Super Admin','Administrator HR','HR','HR Manager'].includes(p.role))setLogged(true)}})},[]);
 ...
 async function login(e:FormEvent){
  ...
  if(pe || !profile || profile.status!=='Aktif' || !['Admin','Super Admin','Administrator HR','HR','HR Manager'].includes(profile.role)){
 ...
 return <div className="talenta-shell">
  <aside className={`talenta-sidebar ${sidebar?'':'collapsed'}`}>
   ...
   <nav className="sidebar-nav">{menuGroups.map(g=><div className="nav-group" key={g.title}>
    ...
    {(sidebar?open[g.title]:true)&&g.items.map(([key,label,icon])=><button key={key} className={`nav-item ${menu===key?'active':''}`} onClick={()=>setMenu(key as MenuKey)} title={label}><span className="nav-icon">{icon}</span>{sidebar&&<span>{label}</span>}{key==='employees'&&sidebar&&<em>{employees.length}</em>}</button>)}
   </div>)}</nav>
